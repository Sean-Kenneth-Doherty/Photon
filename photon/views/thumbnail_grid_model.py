...
class ThumbnailLoader(QRunnable):
    ...
    def run(self):
        thumbnail_path = self.image_processor._generate_thumbnail_sync(
            self.photo_path, (self.size.width(), self.size.height())
        )
        if thumbnail_path and os.path.exists(thumbnail_path):
            self.callback(self.index, QPixmap(thumbnail_path))
        else:
            # Load the error thumbnail
            error_thumbnail_path = os.path.join(
            ...
            self.callback(self.index, QPixmap(error_thumbnail_path))
...
class ThumbnailGridModel(QAbstractListModel):
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> typing.Any:
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            if photo.thumbnail_path and os.path.exists(photo.thumbnail_path):
                ...
                return self._draw_color_label(pixmap, photo.color_label)
            else:
                # Request thumbnail generation asynchronously
                worker = ThumbnailLoader(
    ...
    def _draw_color_label(self, pixmap: QPixmap, color_label: str) -> QPixmap:
        if not color_label or color_label == "None":
            return pixmap

        painter = QPainter(pixmap)
        color = QColor(color_label)
        painter.setBrush(color)
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRect(0, pixmap.height() - 10, pixmap.width(), 10)
        painter.end()
        return pixmap
    ...
    def _on_thumbnail_loaded(self, index: QModelIndex, pixmap: QPixmap):
        photo = self._photos[index.row()]
        if pixmap:
            photo.thumbnail_path = self._image_processor._generate_thumbnail_sync(
                photo.file_path,
                (self.thumbnail_size.width(), self.thumbnail_size.height()),
            )
        else:
            # Use a placeholder for failed thumbnails
            photo.thumbnail_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "..",
                "..",
                "cache",
    ...
    def set_photos(self, photos: List[PhotoMetadata]):
        self.thread_pool.clear() # Clear any pending tasks
    ...
    def set_thumbnail_size(self, width: int, height: int):
        ...
        self.dataChanged.emit(
            ...
            [Qt.ItemDataRole.SizeHintRole, Qt.ItemDataRole.DecorationRole],
        )
